...
fn main() {
    ...
    if !cli.source.as_ref().is_none_or(|src| src == "template" || src == "message") {
        return;
    }
    ...
    let mut msg_file = File::options().read(true).write(true).open(cli.message_file).unwrap();
    let mut message = String::new();
    msg_file.read_to_string(&mut message).unwrap();

    let mut all_lines: Vec<_> = message.lines().collect();
    let (last_empty,_) = all_lines.iter().enumerate().rfind(|(_, line)| line.trim().is_empty()).unwrap();
    ...
    let new_contents = all_lines.into_iter().fold(String::new(), |mut contents, line| {
